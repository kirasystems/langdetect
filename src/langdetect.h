...
class Detector {
    public:
        ...
        static int const WINDOW_SIZE;
        static std::string const UNKNOWN_LANG;

        Detector();
        // Detect and return most likely language.
        Detected detect(char const *data, unsigned int const &length);

        // Detect multiple languages within the text using a windowed approach.
        std::vector<Detected> detect_multiple(char const *data, unsigned int const &length);

    private:
        int trial_;
        double alpha_;
        std::vector<Detected> detect_(std::vector<std::string> &ngrams);
